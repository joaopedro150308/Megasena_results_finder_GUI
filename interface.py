...
from app import logar_whatsapp, iniciar_automacao, preparar_driver
from app import driver, wait, URL, SESSION_ID
        ...
        if verificar_se_os_campos_foram_preenchidos(window, values) is True:
            ...
            if verificar_se_sao_numeros(window, values) is True:
                ...
                if sg.popup_yes_no(f'Você realmente deseja enviar o resultado para este número?\n\n{formatar_numero_de_telefone(values)}') == 'Yes':
                    ...
                    window.write_event_value('iniciando_login', 'Iniciando login')

    # -- Eventos relacionados ao login na conta Whatsapp --
    elif event == 'iniciando_login':
        sg.popup_no_titlebar('A tela de login está iniciando. Por favor aguarde a PRÓXIMA MENSAGEM.', auto_close=True, auto_close_duration=5, non_blocking=False)

        TELEFONE = f'{values["codigo_pais"]}{values["codigo_area"]}{values["numero"]}'
        thread_logar = Thread(target=iniciar_automacao, args=(window, TELEFONE, driver, wait, URL, SESSION_ID, False), daemon=True)
        thread_logar.start()
        ...
        sg.popup_no_titlebar('Login bem suscedido', non_blocking=True, auto_close=True, auto_close_duration=5)
        ...
        thread_logar.join()
        atualizar_disabled_do_elemento(window, 'logar', False)
        sg.popup('Ooops. Erro ao fazer login. Tempo excedido\n\nPor favor tente novamente.')

    # -- Eventos relacionados ao inicio do real processo de automação --
    elif event == 'botao_comecar':
        TELEFONE = f'{values["codigo_pais"]}{values["codigo_area"]}{values["numero"]}'
        thread_automacao = Thread(target=iniciar_automacao, args=(window, TELEFONE, driver, wait, URL, SESSION_ID, True), daemon=True)
        sg.popup_no_titlebar('Iniciando automação!\n\nPor favor aguarde e aproveite ;)', auto_close=True, auto_close_duration=5)
